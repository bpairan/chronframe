 ...
 */
class ToLocalDateTimeConverterSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  val wire = new TextWire(Bytes.elasticHeapByteBuffer(8))
  val Key = "localDateTime"
  private val ExpectedTime = LocalDateTime.of(2003, 2, 1, 14, 5, 2).some
  ...
  it should "be None for null" in {
    ...
    ToLocalDateTimeConverter.convert(wire.read(Key)) shouldBe None
  }
  ...
  it should "convert yyyy/MM/dd HH:mm:ss format" in {
    ...
    ToLocalDateTimeConverter.convert(wire.read(Key)) shouldBe ExpectedTime
  }
